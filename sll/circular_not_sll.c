...
int main()
{
	...
	while(1)
	{
		printf("1.add_last  2.display  3.circular_or_not  4.exit\n");
		...
		switch(choice)
		{
			...
			case 2:display(head);
			       break;
			case 3:if(circular_not(head))
				       printf("list is circular\n");
			       else
				       printf("list not circular\n");
			       break;
			case 4:exit(0);
		}
	}
...
bool circular_not(struct st *ptr)
{
	struct st *temp=NULL;
	if(ptr==NULL)
	{
